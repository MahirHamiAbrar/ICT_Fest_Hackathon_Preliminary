...
def test_21st_request_within_60s_is_rate_limited(client):
    ...
    assert (
        429 not in statuses[:20]
    ), f"first 20 requests must not be rate limited: {statuses}"
    assert (
        statuses[20] == 429
    ), f"21st request must be rate limited, got statuses={statuses}"
...
def test_rate_limit_holds_under_concurrent_requests(client):
    # Run several burst rounds to increase the chance of catching race bugs.
    rounds = 3
    for round_idx in range(rounds):
        admin = new_admin(client, org_prefix=f"rl-round-{round_idx}")
        room = create_room(client, admin)
        ...
        assert len(non_rate_limited) == 20, (
            f"expected exactly 20 admitted requests in round {round_idx}, "
            f"got {len(non_rate_limited)} with statuses={statuses}"
        )
        assert len(rate_limited) == 10, (
            f"expected exactly 10 rate-limited requests in round {round_idx}, "
            f"got statuses={statuses}"
        )
        assert all(
            s in {201, 409, 429} for s in statuses
        ), f"unexpected status in concurrent burst: {statuses}"

    # Keep tests isolated from whichever user ids the rest of the suite creates.
