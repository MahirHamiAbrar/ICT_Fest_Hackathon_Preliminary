...
| `test_quota_holds_under_concurrent_requests` | 8 concurrent requests across 8 different rooms/slots (all quota-eligible, no room conflicts) → exactly 3 succeed, 5 get `409 QUOTA_EXCEEDED`. | **fail — bug** |

## Bugs caught

- **Quota check is not race-safe.**
  `app/routers/bookings.py::_check_quota` counts the caller's existing
  in-window confirmed bookings, then (after an artificial `_quota_audit()`
  delay) the caller inserts a new row — with no locking/serialization
  between the count and the insert. Concurrent requests can all read the
  same "count so far" before any of them commits, so more than 3 bookings
  can be admitted into the window. See `docs/app/routers/bookings.md`.

Note: the sequential tests above (single-threaded, one request at a time)
all pass — the quota *logic* itself (window bounds, per-member scoping,
