...
- `Base.metadata.create_all(bind=engine)` creates database tables from ORM models on startup/import.
- `app = FastAPI(title="CoWork API", version="1.0.0")` creates the ASGI app instance.
- `app.add_exception_handler(AppError, app_error_handler)` wires custom business errors into JSON responses.
- `app.include_router(...)` mounts routers in this order: health, auth, rooms, bookings, admin.
